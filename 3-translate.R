...
# )
# 
# en_f <- foto %>% mutate(country = toEnglish2(
#   text = kraj,
#   # nie działa
#   source_lang = "PL",
#   auth_key = my_key
...
# foto$country <- toEnglish2(
#   text = split_text(foto$kraj, max_size_bytes = 29000),
#   # nie działa
#   source_lang = "PL",
#   auth_key = my_key
# )
...
en_f <- foto %>% mutate(
  ...
  stereotype = case_when(
    stereotyp == "neutralny" ~ "non-reference",
    stereotyp == "obalający" ~ "debunked",
    stereotyp == "utrwalający" ~ "perpetuated"
  )
...
en_p <- en_p %>%
  mutate(post_date = dmy(gsub(
    " r\\.|r\\.", "", replace_months(data_posta)
  )))
...
en_p <- en_p %>%
  mutate(
    intent = factor(
      intent,
      levels = c("discouraging", "neutral", "encouraging"),
      ordered = TRUE
    ),
    stereotype = factor(
      stereotype,
      levels = c("perpetuated", "non-reference", "debunked"),
      ordered = TRUE
    )
  )
